...
public:
	...
	void printBetween(Lending l1, Lending l2);
...
template<typename T>
inline void BTree<T>::printBetween(Lending l1, Lending l2)
{
...
{
	int i = 0;
	for (; i<p->nkeys&&x>p->Key[i]; i++);
	if (i < p->nkeys)
		if (x == p->Key[i])
			return p;
	if (!p->nsons)
		return NULL;
	return search(p->Son[i], T x);

};


template <typename T>
/*void BTree<T>::printAllKeys(BNode<T> *t, std::function<bool(const T&)> predicate)
{
	if (!t->nsons)
		for (int i = 0; i < t->nkeys; i++)
			if(predicate(t->Key[i]))
			cout << t->Key[i];
	for (int i = 0; < i < t->nson; i++)
	{
